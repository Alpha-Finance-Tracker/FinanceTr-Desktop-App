...
def prepare_token_for_request():
    ...
    verification_response = requests.get(f"{login_service}/verify_access_token",
    ...
    if verification_response.status_code == 200:
        ...
        access_token_response = requests.get(f"{login_service}/refresh_access_token",
        ...
        if access_token_response.json()['Validity'] != 'Expires':
            ...
            refresh_token_response = requests.get(f"{login_service}/refresh_refresh_token",
                                                  headers={'Authorization': f'Bearer {refresh_token}'})
...
def check_if_login_is_needed():
    refresh_token = retrieve_token('FinanceTr_Refresh_token')
    response = requests.get(f"{login_service}/verify_refresh_token",
                            headers={'Authorization': f'Bearer {refresh_token}'})
